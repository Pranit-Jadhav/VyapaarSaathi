...
def _simple_denoise(audio: np.ndarray, sample_rate: int = 16000) -> np.ndarray:
    """
    Lightweight denoise: attenuate low-energy frames to reduce constant hiss.
    This is not a full noise suppressor but helps improve ASR stability.
    """
    if audio.size == 0:
        return audio

    frame_len = int(0.02 * sample_rate)  # 20ms frames
    if frame_len <= 0 or audio.shape[0] < frame_len:
        return audio

    frames = audio[: audio.shape[0] - (audio.shape[0] % frame_len)].reshape(-1, frame_len)
    rms = np.sqrt(np.mean(frames**2, axis=1) + 1e-8)
    noise_floor = float(np.percentile(rms, 20))
    if noise_floor <= 0:
        return audio

    attenuate_mask = rms < (noise_floor * 1.2)
    gain = np.where(attenuate_mask, 0.3, 1.0)
    gain = np.repeat(gain, frame_len)
    processed = audio.copy()
    processed[: gain.shape[0]] *= gain
    return processed


def _apply_vad(audio: np.ndarray, sample_rate: int = 16000) -> np.ndarray:
    """
    Apply VAD to keep voiced regions and drop long silence.
    Falls back to input audio if VAD is unavailable or no speech detected.
    """
    if webrtcvad is None or audio.size == 0:
        return audio

    vad = webrtcvad.Vad(max(0, min(VAD_AGGRESSIVENESS, 3)))
    frame_ms = 30
    ...
    frames = pcm[: pcm.shape[0] - (pcm.shape[0] % frame_len)].reshape(-1, frame_len)

    voiced_flags = []
    for frame in frames:
        voiced_flags.append(vad.is_speech(frame.tobytes(), sample_rate))

    if not any(voiced_flags):
        return audio

    # Include 1-frame padding around voiced frames to reduce clipping of words.
    padded_flags = voiced_flags[:]
    for i, flag in enumerate(voiced_flags):
        if flag:
            if i > 0:
                padded_flags[i - 1] = True
            if i + 1 < len(padded_flags):
                padded_flags[i + 1] = True

    voiced_frames = frames[np.array(padded_flags)]
    if voiced_frames.size == 0:
        return audio

    voiced_pcm = voiced_frames.reshape(-1)
    return voiced_pcm.astype(np.float32) / 32767.0
...
def _preprocess_audio(file_path: str) -> Optional[str]:
    ...
    # Apply lightweight denoise and VAD trimming to reduce background noise.
    audio = _simple_denoise(audio, sample_rate=16000)
    audio = _apply_vad(audio, sample_rate=16000)
